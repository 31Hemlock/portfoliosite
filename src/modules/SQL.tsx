...
export const SQLTabContent: TabContentData = {
  ...
  content: (
    <>
      <ContentCard>
        <p className={`${mainHeaderClasses}`}>{SQLCoreTabContent.subtitle}</p>
        {divider}
        <p className={`${paragraphClasses}`}>
          For a college project I created an SQL database, complete with
          triggers, procedures, and an EER diagram that demonstrates the
      ...
      <ContentCard>
        ...
        {divider}
        <p className={`${paragraphClasses}`}>
          The SQL file can be found{' '}
          <LinkWrapper url={`${CodeLink}SQL`} text="here" />.
        </p>
      </ContentCard>
    </>
  ),
