...
const MobileSidebar: React.FC<MobileSidebarProps> = ({mobileShowMenu, setMobileShowMenu}) => {
    ...
    return (
        <>
            <div className="p-6 pt-3 right-0 cursor-pointer w-fit absolute" onClick={() => toggleMobileShowMenu()}> {/* Clickable area of the menu */}
                <img className=" w-[32px] h-[32px]  " src={threeBarsUrl} alt="Menu" />
            </div>
            <div className="w-full mx-auto px-6 py-2">
                <Link to='/portfoliosite' className="text-header-white mx-auto text-center block text-3xl  hover:text-hover-highlight w-max " onClick={() => setMobileShowMenu(false)}>Noah Zaranka</Link> 
            </div>
            <div className={`px-16 grid transition-all duration-[400ms] ${mobileShowMenu ? "grid-rows-[1fr] opacity-100 blur-none" : "grid-rows-[0fr] opacity-0 blur-xl"}`}>
